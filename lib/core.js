...
module.exports = function SPCore(mod) {
	...
	// Reaper Cable Step fix
	const csEmpty = new Vec3(0, 0, 0)
	
	mod.hook('C_START_TARGETED_SKILL', 7, event => {
		if(csEmpty.dist3D(event.dest) <= 1) {
			if(player.job === 8 && event.skill.id === 200100) {
	...
	function handleStartSkill(type, event, info, data) {
		...
		if((fhlock && player.job === 6 && event.skill.id === 193200) || (ohsLock && player.job === 2 && skillBase === 8)) {
			sendCannotStartSkill(event.skill)
			return false
		}
		...
		if(!fhlock && player.job === 6 && event.skill.id === 193210) {
			fhlock = true
			mod.setTimeout(()=>{ fhlock = false }, 2600)
		}
		
		if(!ohsLock && player.job === 2 && skillBase === 8) {
			ohsLock = true
			mod.setTimeout(()=>{ ohsLock = false }, 700)
		}
	...
	function getServerTimeout() {
		return ping.last + (mod.settings.skills.retryCount * mod.settings.skills.retryMs) + (chargeSkill ? 9999 : mod.settings.skills.serverTimeout)
	}
}

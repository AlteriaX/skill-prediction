...
class SPAbnormals {
	constructor(mod) {
		...
		let abnormalityUpdate = (type, event) => {
			if(event.target === this.player.gameId) {
				...
				if(event.duration != 0x7fffffffn && !this.blacklist[event.id]) event.duration = BigInt(Math.round(Math.max(Number(event.duration) - this.ping.last, 0)))
		...
		mod.hook('S_ABNORMALITY_BEGIN', 4, abnormalityUpdate.bind(null, 'S_ABNORMALITY_BEGIN'))
	...
	add(id, duration, stacks) {
		let type = this.myAbnormals[id] ? 'S_ABNORMALITY_REFRESH' : 'S_ABNORMALITY_BEGIN',
			version = this.myAbnormals[id] ? 2 : 4
